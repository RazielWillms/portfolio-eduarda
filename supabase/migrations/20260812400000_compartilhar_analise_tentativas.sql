-- Compartilhamento opcional de agregados de tentativas, sem observacoes ou itens individuais.
...
declare v record;
begin
  select * into v from public.criar_acesso_responsavel(p_paciente_id,p_validade_dias,p_descricao,p_escopo,p_periodo_meses,p_alvos,p_exibir_criterios,p_exibir_fases,p_exibir_integridade,p_exibir_contextos);
  update public.acessos_responsavel a set configuracao=a.configuracao||jsonb_build_object('exibir_analise_tentativas',p_exibir_analise_tentativas) where a.id=v.id;
...
declare v_base jsonb;v_alvos jsonb;
begin
  ...
  select coalesce(jsonb_agg(jsonb_set(alvo,'{pontos}',coalesce((
    select jsonb_agg(jsonb_set(ponto,'{tentativas}',coalesce((
      select jsonb_build_object(
        ...
        'ajudas',jsonb_build_object('gestual',count(*)filter(where t.nivel_ajuda='gestual'),'verbal',count(*)filter(where t.nivel_ajuda='verbal'),'modelo',count(*)filter(where t.nivel_ajuda='modelo'),'fisica_parcial',count(*)filter(where t.nivel_ajuda='fisica_parcial'),'fisica_total',count(*)filter(where t.nivel_ajuda='fisica_total'))
      )from public.tentativas_individuais t where t.registro_medicao_id=(ponto->>'id')::uuid having count(*)>0
    ),'null'::jsonb),true))from jsonb_array_elements(alvo->'pontos')ponto
  ),'[]'::jsonb),true)))into v_alvos from jsonb_array_elements(v_base->'alvos')alvo;
  return jsonb_set(v_base,'{alvos}',coalesce(v_alvos,'[]'::jsonb),true);
end $$;

revoke all on function public.criar_acesso_responsavel_v2(uuid,integer,text,text,integer,uuid[],boolean,boolean,boolean,boolean,boolean)from public;
