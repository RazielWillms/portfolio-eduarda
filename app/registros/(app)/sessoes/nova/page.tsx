...
export default async function NovaSessaoPage({ searchParams }: { searchParams: Promise<{ paciente?: string;agendamento?:string }> }) {
  const { paciente: pacienteId,agendamento } = await searchParams
  ...
  return <div className="space-y-6"><div><h1 className="text-2xl font-bold">Nova sessão estruturada</h1><p className="text-sm text-muted-foreground">Registre vínculo, avaliação, linha de base ou intervenção conforme o momento clínico.</p></div><SessaoClinicaForm paciente={paciente} alvos={alvos} agendamentoId={agendamento}/></div>
}
