...
describe("sequências de faltas não justificadas",()=>{
 ...
 it("leva a sequência ao relatório completo usado pelo PDF",()=>{expect(sql).toContain("create or replace function public.relatorio_frequencia(");expect(sql).toContain("'sequencia_quantidade',o.sequencia_quantidade")})
