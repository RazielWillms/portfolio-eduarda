...
import type { CandidatoDuplicataPaciente, ContinuidadeFalta, ContextoFaltaAnterior, Papel, TipoOcorrenciaFrequencia } from "./types"
...
export async function consultarContextoFaltaAnterior(input:{pacienteId:string;profissionalId:string;data:string}){const supabase=await createClient(),{data,error}=await supabase.rpc("contexto_falta_anterior",{p_paciente_id:input.pacienteId,p_profissional_id:input.profissionalId,p_data:input.data});if(error){reportServerError("consultarContextoFaltaAnterior",error);return genericError("Não foi possível consultar o histórico de faltas.")}return{success:true,data:(data as ContextoFaltaAnterior|null)}}
export async function registrarOcorrenciaFrequencia(input:{pacienteId:string;profissionalId:string;data:string;tipo:TipoOcorrenciaFrequencia;motivo:string;observacao:string;agendamentoId?:string|null;continuidadeFalta?:ContinuidadeFalta|null;ocorrenciaAnteriorId?:string|null}){const supabase=await createClient(),{error}=await supabase.rpc("registrar_ocorrencia_frequencia",{p_paciente_id:input.pacienteId,p_profissional_id:input.profissionalId,p_data:input.data,p_tipo:input.tipo,p_motivo:input.motivo,p_observacao:input.observacao,p_agendamento_id:input.agendamentoId||null,p_continuidade_falta:input.continuidadeFalta||null,p_ocorrencia_anterior_id:input.ocorrenciaAnteriorId||null});if(error){reportServerError("registrarOcorrenciaFrequencia",error);if(error.message.includes("stale_previous_occurrence"))return genericError("O histórico de faltas mudou. Revise a continuidade antes de registrar.");if(error.message.includes("duplicate_schedule_occurrence")||error.code==="23505")return genericError("Este agendamento já possui uma ocorrência de frequência.");if(error.code==="42501")return genericError("Você não pode registrar uma ocorrência para este paciente ou profissional.");if(error.code==="22023")return genericError("Revise a data, o tipo, o motivo e a continuidade informada.");if(["42883","PGRST202"].includes(error.code??""))return genericError("A atualização de sequências de faltas ainda não foi instalada no banco.");return genericError("Não foi possível registrar a ocorrência.")}revalidatePath("/registros/frequencia");revalidatePath("/registros/agenda");revalidatePath("/registros");return{success:true}}
