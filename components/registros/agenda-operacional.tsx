...
function podeRegistrarSessao(inicio: string) { return new Date(inicio).getTime() <= Date.now(); }
export function AgendaOperacional({ agendamentos, opcoes, profissoes, papel, usuarioId, referencia, visao, destaqueId }: {
    ...
    function navegar(dias: number) { const d = new Date(dataReferencia); d.setDate(d.getDate() + dias); router.push(`/registros/agenda?data=${d.toLocaleDateString("en-CA")}&visao=${visao}`); }
